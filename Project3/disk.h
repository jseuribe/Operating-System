...
class disk
{
	...
	public:
		...
		bool kill_process(PCB process, PCB &data);

		bool empty();//true if both empty, false if a process exists.
...
void disk::print_queue_left(LinkedList<PCB> request, LinkedList<PCB> frozen)
{
	...
	cout << "Lg" << "          " << "Phys" << "         " << "PID" << "        " << "FName" << "      " <<  "FLoc"<< "       " << "FSiz" 
         << "       " << "r/w" << "        " << "NxBrst" <<  "     " << "Mem" << endl;
...
bool disk::kill_process(PCB process, PCB &data)
{
	if( queues[0].find(process, data) ) return true;
	else if( queues[1].find(process, data)) return true;
	else return false;
}
