...
public:
	file();
	file(string &name, char &new_op, int &size, string &loc);
	file(const file& inp);
	file& operator=(const file& inp);
	string return_loc() const;
	void change_parameters(string &new_name, char &new_file_op, int &new_size, string &new_loc);
